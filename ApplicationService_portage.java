...
public class ApplicationService {
    ...
    public UUID changeApplicationConfiguration(
        ...
        try {
            progression1 = (ReactiveProgression.ChangeApplicationProgression) changeApplicationConfiguration(
                    comment,
                    progression1,
                    application,
                    dataFile,
    ...
    private ReactiveProgression.ChangeOrCreateApplicationProgression changeApplicationConfiguration(
        ...
        if (Objects.requireNonNull(configurationFile.fileName()).matches(".*\\.zip")) {
            ...
            newApplication = ApplicationConfigurationService.parseConfigurationBytes(applicationName, comment, progressionForConfiguration, FileBomResolver.of(multiYAmlInput));
        } else {
            progressionForParsingConfiguration.pushMessage("forSingle", Map.of(APPLICATION_NAME, applicationName));
            newApplication = ApplicationConfigurationService.parseConfigurationBytes(applicationName, comment, progressionForConfiguration, FileBomResolver.of(configurationFile.inputStream()));
        }
        if (newApplication == null) {
            return progression;
        }
        progression.fluxSink().next(new ReactiveTypeInfo<>("oldApplication.configuration.create.register.start", Map.of(APPLICATION_NAME, applicationName)));
        ...
        try {
            ...
            //newApplication = createOrModifySchema.apply(newApplication);
    ...
    public Application validateConfiguration(final ReactiveProgression.CreateApplicationProgression fluxSink, final DataFile file) {
        try {
            ...
            try {
                ...
                fluxSink.pushMessage("migration",
                        Map.of(
                                APPLICATION_NAME, oldApplication.getName(),
                                MIGRATION_REPORT, new JsonRowMapper<>().toJson(migrationResult)
                        )
                );
            } catch (Exception e) {
                //nothing to do
