...
class GroovyCheckerExampleBuilder {
    protected static final GroovyCheckerType T_11 = buildGroovyChecker("""
            ...
            new LinkedHashMap<>() {{
                put("BAD_VALUE", new I18nType(
                        new LinkedHashMap<>() {
                            {
                                put("fr", "la valeur ${value} doit être l'une des valeurs de ${values}");
                                put("en", "value ${value} must be in ${values}");
                            }}
                        )
                );
            }},
    ...
    protected static final CheckerType INTERVAL_DATE = buildGroovyChecker("""
                    if (datum.data_dat == null) {
                                                ...
                                                return true;""",
            new LinkedHashMap() {{
                put("MISSING_DATE", new I18nType(
                        new LinkedHashMap<>() {{
                            put("fr", "la date est manquante");
                            put("en", "missing date");
                        }}
                ));
                put("DATE_NOT_IN_INTERVAL", new I18nType(
                        new LinkedHashMap<>() {{
                            put("fr", "la date ${date} n'est pas dans l'intervale de dates [${dateDebut},${dateFin}]");
                            put("en", "the date ${date} is not in date intervale [${dateDebut},${dateFin}]");
                        }}
                ));
                put("BAD_DATE_FORMAT", new I18nType(
                        new LinkedHashMap<>() {{
                            put("fr", "la date ${date} n'est pas au format ${format}");
                            put("en", "the date ${date} is not in format ${format}");
                        }}
                ));
            }},
            Multiplicity.ONE
