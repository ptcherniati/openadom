...
public enum ViewStrategy {
    ...
    public static SiOreIllegalArgumentException getError(final ViewStrategy viewStrategy) {
        return new SiOreIllegalArgumentException(
                "badViewStrategie",
                Map.of(
                        "viewStrategy", viewStrategy,
                        "knownViewStrategies", Arrays.stream(values()).map(ViewStrategy::toString).collect(Collectors.toSet())
                )
        );
    }
