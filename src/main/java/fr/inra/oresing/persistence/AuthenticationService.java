...
public class AuthenticationService implements AuthenticationServiceImpl, AuthenticationPort {
     ...
     * 2-phases : la cle est derivee de ( userId , nouvelEmail , updateDate ) ;
     * a la phase 1 user.email pointe encore sur l'ancien email , a la phase 2
     * le frontend re-fournit le nouvel email avec la cle et le backend
     * recompute la cle attendue pour la comparer .
     *
     ...
     * <ul>
     *     <li>{@code verificationKey} present ET {@code email} different de
     *         {@code user.email} -> phase 2 d'un changement d'email :
     *         {@link #commitEmailChange} valide la cle ( recompute attendue
     *         depuis le couple userId+nouvelEmail+updateDate ) et ecrase
     *         directement {@code email} ;
     *     <li>{@code newPassword} present : applique le changement de mdp ;
     *     <li>{@code email} different de user.email SANS {@code verificationKey} :
     *         phase 1 d'un changement d'email - envoie la cle au nouvel email
     *         et n'effectue AUCUN stockage intermediaire ( la colonne
     *         {@code pending_email} n'existe plus ) ; le frontend conserve
     *         le nouvel email en memoire JS et le re-fournit en phase 2 ;
     *         {@code email} et {@code accountstate} INCHANGES pour que
     *         l'utilisateur reste pleinement actif pendant la validation .
     * </ul>
     */
    private OreSiUser updateAccount(final OreSiUser user, final CreateUserRequest createUserRequest) throws AuthenticationFailure, JsonProcessingException {
        ...
        // Phase 2 d'un changement d'email : cle de validation presente ET
        // l'appelant cible un email different . Sans pending_email en base ,
        // c'est le couple ( email cible , cle ) fourni par le frontend qui
        // permet au backend de recomputer la cle attendue et de la comparer .
        final boolean hasVerificationKey = !Strings.isNullOrEmpty(createUserRequest.getVerificationKey());
        if (hasVerificationKey && emailChanged) {
            return commitEmailChange(user, requestedEmail, createUserRequest.getVerificationKey());
        }
        ...
        if (emailChanged) {
            ...
            //   1. compute key bound to nouvel email ( in-memory )
            //   2. send mail SMTP -> throws MailServiceUnavailableException si KO
            //   3. mail OK -> persist updateDate ( ancrage TTL )
            // Aucun stockage du nouvel email en base : le frontend conserve la
            // valeur en memoire JS et la re-fournit en phase 2 ; le backend
            // recompute alors la cle attendue depuis ( userId , nouvelEmail ,
            // updateDate ) et la compare avec celle recue . En cas d'erreur
            // SMTP a l'etape 2 , aucune mutation DB n'a ete appliquee -> retry
            // direct utilisateur sans etat corrompu .
        ...
        // de mot de passe . L'appelant a effectue une requete vide - typiquement
        // frontend phase 1 avec newEmail == email courant . On NE DOIT PAS
        // retourner 200 sinon le frontend afficherait un toast vert "mail
        // envoye" alors qu'aucun mail n'a ete envoye . Invariant : 200 OK
        // <=> mail envoye .
        if (createUserRequest.getNewPassword() == null) {
    ...
    /**
     * Phase 2 d'un changement d'email : recompute la cle attendue depuis
     * ( userId , {@code newEmail} fourni par le frontend , {@code updateDate} )
     * et la compare avec celle saisie par l'utilisateur ; si match , ecrase
     * directement {@code email} avec {@code newEmail} . En cas de cle
     * invalide ou expiree , aucune modification n'est appliquee .
     *
     * <p>Sans stockage intermediaire en base ( colonne {@code pending_email}
     * supprimee ) : c'est le frontend qui transporte le nouvel email entre
     * la phase 1 ( envoi de la cle ) et la phase 2 ( soumission de la cle ) .
     * Si l'utilisateur modifie son saisie de nouvel email entre les deux
     * phases sans redemander une cle , la cle recomputee diverge automa-
     * tiquement et la validation echoue ( comportement attendu ) .
     *
     * @throws AuthenticationFailure BAD_VALIDATION_KEY si la cle est invalide
     *         ou expiree ; EXISTING_EMAIL si quelqu'un a pris l'email cible
     *         entre les deux phases .
     */
    private OreSiUser commitEmailChange(final OreSiUser user, final String newEmail, final String verificationKey) throws AuthenticationFailure, JsonProcessingException {
        final String expected = generateVerificationKey(user, newEmail);
        ...
        // un autre utilisateur peut avoir pris l'email cible . Sans ce recheck ,
        // la contrainte unique {@code oresiuser_email_key} declencherait une
        // violation SQL -> 500 cryptique . On detecte la collision tot pour
        // rendre une erreur metier explicite .
        final Optional<OreSiUser> collidingUser = userRepository.findByEmail(newEmail);
        if (collidingUser.isPresent() && !collidingUser.get().getId().equals(user.getId())) {
            throw new AuthenticationFailure(AuthenticationFailure.EXISTING_EMAIL, collidingUser.get());
        }
        setRoleAdmin();
