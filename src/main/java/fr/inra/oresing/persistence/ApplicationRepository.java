...
public class ApplicationRepository extends JsonTableRepositoryTemplate<Application> {
    ...
    protected String getUpsertQuery() {
        return "INSERT INTO " + getTable().getSqlIdentifier() + " (id, name, comment, referenceType, dataType, additionalFile, configuration, configFile) SELECT id, name,  comment, referenceType, dataType, additionalFile, configuration, configFile FROM json_populate_recordset(NULL::" + getTable().getSqlIdentifier() + ", :json::json)"
                + " ON CONFLICT (id) DO UPDATE SET updateDate=current_timestamp, comment=EXCLUDED.comment, name=EXCLUDED.name, referenceType=EXCLUDED.referenceType, dataType=EXCLUDED.dataType, additionalFile=EXCLUDED.additionalFile, configuration=EXCLUDED.configuration, configFile=EXCLUDED.configFile"
                + " RETURNING id";
    }
