...
export default class AdditionalFilesManagementView extends Vue {
  ...
  async deleteAdditionalFile(event) {
    var fileTodelete = this.additionalFiles.find((file) => file.id == event) || {};
  ...
  async doDeleteAdditionalFile(fileTodelete) {
    ...
    this.additionalFiles = this.additionalFiles.filter((file) => !uuid.includes(file.id));
  }
  ...
  getButtons(fileId) {
    ...
    if (this.application.isAdministrator || fileId == this.currentUserId) {
      buttons.push(
        new Button(
          this.$t("additionalFiles.buttons.delete"),
          "times",
          (label) => this.deleteAdditionalFile(label),
          "is-danger"
        )
      );
    }
