  ...
  <PageView class="with-submenu">
    ...
    <div>
      <b-select :placeholder="filterStates[filterState].label" v-model="filterState">
        <option v-for="(option, id) in filterStates" :key="id"
                :value="id"
        >
          {{ option.label }}
        </option>
      ...
      <div
          v-for="(rightsRequest, i) in rightsRequests.rightsRequests"
          :key="i">
        <CollapsibleTree
        ...
        >
          ...
          <template v-slot:label="{option}">
            <b-icon :icon="option.setted?'check':'clock'" :type="option.setted?'is-primary':'is-light'"
                    class="column is-one-fifth">
              {{ option.setted ? $t('dataTypeAuthorizations.scopes.close') : $t('dataTypeAuthorizations.scopes.open') }}
            </b-icon>
            <div class="column is-four-fifth">{{ option.comment || '---' }}</div>
          </template>
          <template v-slot:synthesisDetail="{option}">
            <div class="column is-one-fifth">{{ (option.creationDate && getDate(option.creationDate)) || '---' }}</div>
...
export default class RequestAuthorizationManagementView extends Vue {
  ...
  buttons = [
    new Button(
        this.canManageRights ? this.$t("dataTypeAuthorizations.grantRequests") : this.$t("dataTypeAuthorizations.modifyRequests"),
        "eye",
  ...
  async init() {
    try {
      ...
      users1 = users1.filter(user => {
        return rightsRequests.rightsRequests
            .some(rr => rr.user == user.id)
      })
