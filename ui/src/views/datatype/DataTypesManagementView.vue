...
export default class DataTypesManagementView extends Vue {
  ...
  buttons(dty) {
    return [
      ...
      new Button(
        this.$t("referencesManagement.download"),
        "download",
        (label) => this.downloadDataType(label),
        null,
        null,
        !dty.canDownload
      ),
  ...
  async init() {
    try {
      ...
      this.dataTypes = Object.values(
        ...
        let authorizationsDatatypesRights =
          this.application.authorizationsDatatypesRights[dty.label];
  ...
  async downloadDataType(event) {
    ...
    let path = this.dataService.getDataTypesCsvPath(this.applicationName, event, param);
    let link = document.createElement("a");
    link.href = path;
    link.download = this.dataTypeId + ".zip";
    link.click();
