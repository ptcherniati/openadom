...
export default class ReferencesManagementView extends Vue {
  ...
  async init() {
    try {
      ...
      this.references = convertReferencesToTrees(
        Object.values(this.internationalisationService.treeReferenceName(this.application)).map(
          (ref) => {
            let isAdmin =
              this.application.authorizationReferencesRights.authorizations[ref.label].ADMIN;
            let canUpload =
              isAdmin ||
              this.application.authorizationReferencesRights.authorizations[ref.label].UPLOAD;
            let canRead =
              isAdmin ||
              this.application.authorizationReferencesRights.authorizations[ref.label].UPLOAD;
            let canDownload =
              isAdmin ||
              this.application.authorizationReferencesRights.authorizations[ref.label].DOWNLOAD;
            let canDelete =
              isAdmin ||
              this.application.authorizationReferencesRights.authorizations[ref.label].DELETE;
            let any =
              isAdmin ||
              this.application.authorizationReferencesRights.authorizations[ref.label].ANY;
            return {
              ...ref,
  ...
  findReferenceByLabelOnElement(element, label) {
    ...
    var ref = element
      .map((ref) =>
        ref.label === label ? ref : this.findReferenceByLabelOnElement(ref.children, label)
      )
      .find((t) => t);
    return ref;
  }
