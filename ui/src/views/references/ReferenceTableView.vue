    ...
    <div v-if="reference && columns">
      ...
      >
        ...
        <b-table-column
            ...
            sortable
        >
          <span v-if="column.linkedTo === null && (info(column.id) || multiplicity(column.id, props.row[column.id]))">
            <b-button
            ...
            >
              <span class="tile">
                {{
                  props.row[column.id] &&
                  Object.values(props.row[column.id]).filter((value) => value.length).length
                }}</span
              >
            </b-button>
              ...
              <div class="card">
                ...
                <div class="card-content">
                  <div v-for="key in modalArrayObj" :key="key.id" class="columns modalArrayObj">
                    ...
                    <p v-if="key.value" class="column">{{ key.value }}</p>
                  </div>
                </div>
              </div>
            </b-modal>
          ...
          <span v-else-if="column.linkedTo !== null">
            <a @click="getCheckerReferenceValues(referenceValues[tableValues.indexOf(props.row)].naturalKey, column.linkedTo)">{{ props.row[column.id] }}</a>
          </span>
...
export default class ReferenceTableView extends Vue {
  ...
  async getCheckerReferenceValues(rowId, refType) {
    let checkerId = "";
  ...
  async setInitialVariables() {
    ...
    //let test = await this.getCheckerReferenceValues('d2ae47d9-dfce-4a2a-b147-b3cc647b9f5d', 'tool_type');
    //console.log(test)
    let dynamicColumns = Object.entries(this.reference.dynamicColumns).filter((a) => a[1]);
    for (let i = 0; i < dynamicColumns.length; i++) {
