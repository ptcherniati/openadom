...
export class Fetcher {
    ...
    async get(url, params = {}, isText) {
        ...
        console.log("response", response, "isText", isText)
        return this._handleResponse(response, isText);
    }

    async getBlob(url, params = {}, isText) {
        const path = new URL(url, config.API_URL);
    ...
    async _handleResponse(response, isText) {
        try {
            if ('blob' == isText) {
                ...
                return Promise.reject({httpResponseCode: response.status, content: Promise.resolve(response.text())});
            } else {
                let text =
