<template>
  ...
  >
    <b-field
        ...
        :message="errors"
        :label="label">
      <b-select v-if="references"
                :multiple="multiplicity"
                :required="required"
                @blur="updateValue"
                @input="updateValue"
                :value="val">
        <option v-for="(option) in references" :key="option.naturalKey" :label="getFullName(option.naturalKey)" :value="option.naturalKey">
        </option>
      </b-select>
    </b-field>
  </ValidationProvider>
  ...
  methods: {
    getFullName(naturalKey) {
      ...
      return naturalKey.split('__')
          ...
          .map(reference => reference.values['__display_' + this.defaultLanguage] || reference.naturalKey)
          .join('/') || naturalKey
    },
    extend,
    updateValue(event) {
