...
export default {
  ...
  methods: {
    groupsAuthorizations(auths, ...args) {
      args.forEach(arg => {
        if (arg) {
          arg.filter(pub => auths.indexOf(pub.path) < 0).forEach(pub => {
            auths.push(pub);
            this.ownAuthorizationsColumnsByPath[pub.path] = this.ownAuthorizationsColumnsByPath[pub.path] || []
    ...
    async init() {
      ...
      if (!this.isApplicationAdmin) {
        ...
        if (JSON.parse(localStorage.getItem("authenticatedUser"))) {
          ...
          this.isApplicationAdmin = ownAuthorizations.find((a) => new RegExp(a).test(this.dataType));
        }
        for (const scope in auths) {
          ...
          if (this.ownAuthorizations.indexOf(path) === -1) {
            if (!this.ownAuthorizations.find((pa) => this.comparePathes(path, pa))) {
              this.ownAuthorizations = this.ownAuthorizations.filter((pa) => !this.comparePathes(pa, path));
        ...
        for (const scope in auths) {
          ...
          for (const scopeAuthorizationsKey in scopeAuthorizations) {
            ...
            if (this.publicAuthorizations[scope].indexOf(path) === -1) {
              if (!this.publicAuthorizations[scope].find((pa) => {
