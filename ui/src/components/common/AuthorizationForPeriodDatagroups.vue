<template>
  <div>
      ...
      <template v-slot:content>
        ...
        <div v-else class="has-background-primary show-detail-for-selected">
          ...
          <div>
            ...
            <div v-if="state.from || state.to">
                      <span v-if="state.from">
                        du {{ state.from.toDateString() }}
                      </span>
              <span v-if="state.to">
                        jusqu'au {{ state.to.toDateString() }}
                      </span>
            </div>
          ...
          <div>
            ...
            <div v-if="state.dataGroups &&
                         state.dataGroups.length>0">
                        <span v-for="(datagroup, i) in  state.dataGroups"
                              class="tag is-info"
                              :key="i">
    ...
    <b-modal v-if="currentAuthorization" v-model="showModal" class="modalCardRef" width="70%"
...
export default class AuthorizationForPeriodDatagroups extends Vue {
  ...
  init() {
    ...
    if (authorizations) {
      authorizations.dataGroups = this.dataGroups
          .filter(dg => authorizations.dataGroups.find(dg2 => dg.id == dg2))
      ...
      this.currentAuthorization = {index: this.index, indexColumn: this.indexColumn, authorizations}
    }
  }
