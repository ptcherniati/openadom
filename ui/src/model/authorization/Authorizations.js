...
export class Authorizations {
      ...
      for (const scope in auths) {
        ...
        for (const scopeAuthorizationsKey in scopeAuthorizations) {
          ...
          if (publicAuthorizationToReturn[scope].indexOf(path) === -1) {
            if (!publicAuthorizationToReturn[scope].find((pa) => path.startsWith(pa))) {
  ...
  static async initAuthReferences(
    configuration,
    authorizations,
    authorizationScopes,
    getOrLoadReferences
    ...
    for (const datatype in authorizationScopes) {
      ...
      var remainingAuthorizations = await Authorizations.remainingAuthorizations(
        ret,
        getOrLoadReferences
      );
