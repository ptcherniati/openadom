...
export default class AvailiblityChart extends Vue {
  ...
  mounted() {
    ...
    this.canvasW = this.canvas.width*.9;
  ...
  writeDates(){
    ...
    if ((max.getYear()-min.getYear())>10){
      ...
      formatDate = date => (date.toLocaleDateString()).slice(3,6)+(date.toLocaleDateString()).slice(8,10)
    ...
    for (let i = 0; i <= 10; i++) {
      ...
      let inf = (
        ...
        this.ctx.moveTo(inf + this.canvasW * .05, this.canvasH / 2.5);
        this.ctx.lineTo(inf + this.canvasW * .05, -this.canvasH);
        this.ctx.stroke();
      }
    }
  };
   drawIntervale(range){
    ...
    let inf = (
        (new Date(range[0]).getTime()-min.getTime())/
        (max.getTime()-min.getTime())
    )*this.canvasW
    let sup = (
        (new Date(range[1]).getTime()-min.getTime())/
        (max.getTime()-min.getTime())
    )*this.canvasW
    ...
    this.ctx.moveTo(inf+ this.canvas.width*.05, this.canvasH/4);
    this.ctx.lineTo(sup+ this.canvas.width*.05, this.canvasH/4);
    this.ctx.stroke();
